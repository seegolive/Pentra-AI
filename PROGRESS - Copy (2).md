...
| ST-07: vuln_hunt phase | ⏳ | Tool binaries belum install |
| ST-08: report_node + findings persist | ⏳ | Belum ditest |
...
| # | Task | Keterangan |
|---|------|------------|
| 1 | **Install Go + tool binaries** | `sudo snap install go --classic` → subfinder, httpx, nmap, nuclei, ffuf, dll. |
| 2 | **ST-07/ST-08 smoke test** | vuln_hunt + report_node end-to-end (butuh binary) |
| 3 | **FindingsTable.tsx** | Komponen tabel findings di `EngagementDetailPage` |
| 4 | **ReportViewer.tsx** | Tampilkan markdown report dari agent |
| 5 | **Audit worker tasks** | `cve_enrichment`, `bugcrowd_scraper`, `payloads_all_things`, `rss_ingestion` |
| 6 | **`pentra-payload` integration** | `generator.py` + `models.py` ada tapi belum dipakai agent |
| 7 | **Sprint 12** | HITL frontend real-time — live engagement dashboard + approval flow |
